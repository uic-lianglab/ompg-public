...
BASE_FRAG_LIB_INPUT_DIR=$BASE_OUTPUT_DIR/frag_libs/e_pca_libs
BASE_FRAG_LIB_OUTPUT_DIR=$BASE_OUTPUT_DIR/frag_libs/f_clust_libs
# Log directories
BASE_LOG_DIR=$DATA_DIR/logs
BASE_FRAG_LIB_LOG_DIR=$BASE_LOG_DIR/frag_libs/f_clust_libs
# Executables
EXE_DIR=$ROOT_DIR
EXE_NAME=scoby
...
do
    ...
    # If bin count > 0, then this directory is a frag lib
    ...
    for fraglib in $d/*.bin
    do
        in_matrix_path="$binfulld/$(basename $fraglib)"
        out_samples_to_clusters_path="$outdir"/"$(basename $fraglib .coords.pca.bin)".s2c.csv
        out_clusters_to_samples_path="$outdir"/"$(basename $fraglib .coords.pca.bin)".c2s.csv
        out_exemplars_path="$outdir"/"$(basename $fraglib .coords.pca.bin)".ex.csv
        
        logpath="$logdir"/"$(basename $fraglib .coords.bin)".shwap.txt
