...
BASE_FRAG_LIB_INPUT_DIR=$BASE_OUTPUT_DIR/frag_libs/d_bin_libs
BASE_FRAG_LIB_OUTPUT_DIR=$BASE_OUTPUT_DIR/frag_libs/e_pca_libs
# Log directories
BASE_LOG_DIR=$DATA_DIR/logs
BASE_FRAG_LIB_LOG_DIR=$BASE_LOG_DIR/frag_libs/e_pca_libs
# Executables
EXE_DIR=$ROOT_DIR
EXE_NAME=scoby
...
do
    ...
    do
        ...
        binpath="$binfulld/$(basename $fraglib)"
        pcapath="$outdir"/"$(basename $fraglib .bin)".pca.bin
        
        logpath="$logdir"/"$(basename $fraglib .bin)".pca.txt
