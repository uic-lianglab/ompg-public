...
# Script munges interaction records into a single CSV master file
...
PY_SCRIPT_PATH=$ROOT_DIR/scripts/munge_NAMD_interactions.py
...
# Only process the top energy ranked samples
MAX_ENERGY_RANK=10000
...
# Iterate over directories containing interaction directories
...
do
    ...
    parent_dir=$(cd "$d"; pwd)
    in_ints_dir="$parent_dir/$target_dir_name"
    
    # Full path to energy rankings
    in_ener_csv_path="$parent_dir/ener.csv"
    
    # Full path to output file
    out_prefix="$parent_dir/$target_dir_name"
    
    # Build command line
    cmd="python $PY_SCRIPT_PATH $in_ints_dir $in_ener_csv_path $out_prefix $MAX_ENERGY_RANK"
    echo "$(timestamp): $cmd"
